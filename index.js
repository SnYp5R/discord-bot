...
Client.on("message", async message => {
    ...
    if(!db.get("Infos_membres").find({id: msgauthorid}).value()){
        ...
        if(userxp[1] >= userpniveau[3]){
            ...
            message.channel.send("GG <@" + message.author.id + ">, vous venez de passer au **niveau " + userniveau[2] + "**, il vous faut maintenant **" + userpniveau[3] + " xp** pour passer au niveau suivant !")
        }
    }
});
